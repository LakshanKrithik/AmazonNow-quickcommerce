  ...
  return (
    <div className="mb-8">
      <div className="flex items-end justify-between mb-4">
        ...
        <button className="text-sm font-medium text-[#007185] hover:text-[#c7511f] hover:underline">
          See all
        </button>
      </div>
...
function ProductCard({ product }) {
  ...
  const handleAdd = () => {
    setQty(1)
    addItem({ ...product, asin: product.name, brand: "", product_name: product.name, score: 0, is_alternative: false })
  }

  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-4 flex flex-col hover:shadow-lg transition-shadow group relative">
      {/* Delivery badge */}
      ...
      <div className="flex items-center gap-1.5 mb-3">
        <div className="flex">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`w-3.5 h-3.5 ${i < Math.floor(product.rating) ? 'fill-[#FFA41C] text-[#FFA41C]' : 'fill-gray-200 text-gray-200'}`}
            />
          ))}
        </div>
      ...
      <div className="mt-auto">
        {qty === 0 ? (
          <button
            onClick={handleAdd}
            className="w-full py-2.5 bg-[#FFD814] hover:bg-[#F7CA00] border border-[#FCD200] rounded-xl text-sm font-bold text-[#0F1111] transition-colors shadow-sm"
          >
            Add to cart
          </button>
        ) : (
